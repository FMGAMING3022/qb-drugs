...
local function ProcessHeroin()
	...
	QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.processing"), 15000, false, true, {
		...
		TriggerServerEvent('qb-drugs:processPoppyResin')
...
RegisterNetEvent('qb-drugs:ProcessPoppy', function()
...
RegisterNetEvent("qb-drugs:processHeroin",function()
	QBCore.Functions.TriggerCallback('qb-drugs:validate_items', function(result)
		if result.ret then
			ProcessHeroin()
		else
...
RegisterNetEvent("qb-drugs:pickHeroin", function()
	local playerPed = PlayerPedId()
	...
	if nearbyObject and IsPedOnFoot(playerPed) then
		...
		QBCore.Functions.Progressbar("search_register", Lang:t("progressbar.collecting"), 10000, false, true, {
			...
			TriggerServerEvent('qb-drugs:pickedUpPoppy')
			isPickingUp = false
