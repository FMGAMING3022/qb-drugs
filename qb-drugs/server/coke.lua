...
RegisterServerEvent('qb-drugs:pickedUpCocaLeaf', function()
	local src = source
...
RegisterServerEvent('qb-drugs:processCocaLeaf', function()
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('qb-drugs:processCocaPowder', function()
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
	...
	if Player.Functions.RemoveItem('coke', Config.CokeProcessing.Coke) then
		if Player.Functions.RemoveItem('bakingsoda', Config.CokeProcessing.BakingSoda) then
			if Player.Functions.RemoveItem('finescale', Config.CokeProcessing.finescale) then
				if Player.Functions.AddItem('coke_small_brick', Config.CokeProcessing.SmallCokeBrick) then
					...
					TriggerClientEvent("inventory:client:ItemBox", source, QBCore.Shared.Items['finescale'], "remove", Config.CokeProcessing.finescale)
				...
				TriggerClientEvent('QBCore:Notify', src, "You Don't Have Finescale", "error")	
			end
		else
			Player.Functions.AddItem('coke', Config.CokeProcessing.Coke)
...
RegisterServerEvent('qb-drugs:processCocaBrick', function()
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
