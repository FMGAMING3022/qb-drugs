...
RegisterServerEvent('qb-drugs:pickedUpCannabis', function()
	local src = source
...
RegisterServerEvent('qb-drugs:processCannabis', function()
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('qb-drugs:rollJoint', function()
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateUseableItem("rolling_paper", function(source, item)
    ...
    TriggerClientEvent('qb-drugs:client:rollJoint', source, 'marijuana', item)
end)